...
# === ตั้งค่า RTL-SDR ===
RTL_SDR_ENABLED = config.get("rtl_sdr_enabled", False)
RTL_SDR_FREQUENCY_HZ = config.get("rtl_sdr_frequency_hz", 144900000) # Default 144.900 MHz
RTL_SDR_GAIN = config.get("rtl_sdr_gain", "auto")
RTL_SDR_SAMPLE_DURATION_SEC = config.get("rtl_sdr_sample_duration_sec", 0.1)
...
def record_until_silent(pyaudio_instance):
    ...
    try:
        ...
        while True:
            ...
            # --- อ่านค่าความแรงสัญญาณจาก RTL-SDR เป็นระยะ ---
            if RTL_SDR_ENABLED and recording and (time.time() - last_rtl_sdr_check_time >= 1.0): # อ่านทุกๆ 1 วินาที (ปรับได้)
                # แปลง FREQUENCY (ถ้าเป็น SCAN หรือ string อื่นๆ) เป็น Hz ก่อน
                current_monitoring_freq_hz = None
                try:
                    # สมมติว่า FREQUENCY ถ้าไม่ใช่ SCAN จะเป็นตัวเลข MHz
                    if FREQUENCY.upper() != "SCAN":
                        current_monitoring_freq_hz = int(float(FREQUENCY) * 1_000_000)
                except ValueError:
                    current_monitoring_freq_hz = RTL_SDR_FREQUENCY_HZ # Fallback to config frequency

                if current_monitoring_freq_hz:
                    strength = get_signal_strength_rtl_power(current_monitoring_freq_hz, RTL_SDR_GAIN, RTL_SDR_SAMPLE_DURATION_SEC)
...
def get_signal_strength_rtl_power(frequency_hz, gain="auto", duration_sec=0.1):
    if not RTL_SDR_ENABLED:
        return None
    try:
        # rtl_power -f <lower>:<upper>:<bin_size> -g <gain> -i <interval> -1 -e <exit_timer>
        # เราต้องการวัดที่ความถี่เดียว ดังนั้น lower และ upper ใกล้เคียงกัน และ bin_size เล็กๆ
        freq_mhz_center = frequency_hz / 1_000_000
        freq_mhz_start = freq_mhz_center - 0.001  # ขอบเขตเล็กๆ รอบความถี่ที่สนใจ
        freq_mhz_end = freq_mhz_center + 0.001
        bin_size_hz = 1000  # 1kHz bin

        # คำสั่ง rtl_power จะทำงานตาม duration_sec แล้ว exit
        command = [
            'rtl_power',
            '-f', f'{freq_mhz_start:.3f}M:{freq_mhz_end:.3f}M:{bin_size_hz}',
            '-g', str(gain),  # gain อาจจะต้องเป็นตัวเลข
            '-i', '1',  # interval สั้นๆ (อาจจะไม่จำเป็นถ้าใช้ -e)
            '-1',  # single shot (อ่านครั้งเดียว)
            '-e', f'{duration_sec}s'  # ให้โปรแกรมจบการทำงานหลังผ่านไปตามเวลาที่กำหนด
        ]

        log(f"📡 [RTL-SDR] Executing: {' '.join(command)}")
        process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        stdout, stderr = process.communicate(timeout=duration_sec + 2)  # เพิ่ม timeout ให้มากกว่า duration_sec

        if process.returncode != 0:
            log(f"⚠️ [RTL-SDR] rtl_power error: {stderr}")
            return None

        # Parse output ของ rtl_power (ตัวอย่าง output: date, time, Hz low, Hz high, Hz step, samples, dBm, dBm, ...)
        # 2025-05-30, 22:00:00, 144999000, 145001000, 1000.00, 1, -50.12, -49.88
        lines = stdout.strip().split('\n')
        if not lines:
            log("⚠️ [RTL-SDR] rtl_power no output.")
            return None
        ...
        for line in lines:
            ...
            parts = line.split(', ')
            if len(parts) > 6:  # คอลัมน์ dBm เริ่มจาก index 6 เป็นต้นไป
                try:
                    # เอาค่า dBm ทุกค่าที่ได้จาก bin ต่างๆ ในช่วงสั้นๆ นั้น
                    for val_str in parts[6:]:
                        dbm_values.append(float(val_str))
                except ValueError:
                    continue  # ข้ามบรรทัดที่ parse ไม่ได้

        if not dbm_values:
            log("⚠️ [RTL-SDR] Could not parse dBm values from rtl_power output.")
            return None

        average_dbm = sum(dbm_values) / len(dbm_values)
        log(f"📡 [RTL-SDR] Average signal strength: {average_dbm:.2f} dBm at {freq_mhz_center:.3f} MHz")
        return average_dbm

    except subprocess.TimeoutExpired:
        log(f"⚠️ [RTL-SDR] rtl_power command timed out after {duration_sec + 2} seconds.")
        if process: process.kill()  # Make sure to kill the process
        return None
    except FileNotFoundError:
        log("❌ [RTL-SDR] rtl_power command not found. Please ensure rtl-sdr is installed and in PATH.")
        return None
    except Exception as e:
        log(f"❌ [RTL-SDR] Error getting signal strength: {e}")
        return None
