...
from PIL import Image # เพิ่ม import Pillow
...
TINYSA_CAPTURE_ENABLED = config.get("tinysa_capture_enabled", True) # เปิด/ปิด การ capture
TINYSA_CAPTURE_ROUND = config.get("tinysa_capture_round", 2)      # รอบที่จะทำการ capture (เช่น 2 หรือ 3)
TINYSA_CAPTURE_SAVE_FOLDER = "tinysa_captures" # โฟลเดอร์สำหรับเก็บรูปที่ capture
os.makedirs(TINYSA_CAPTURE_SAVE_FOLDER, exist_ok=True)
...
def record_until_silent(pyaudio_instance, tinysa_ser=None):
    ...
    tinysa_reading_count = 0  # ตัวนับการอ่านค่าจาก TinySA
        ...
        while True:
            ...
            if not recording:
                if amplitude > THRESHOLD:
                    ...
                    tinysa_reading_count = 0  # รีเซ็ตตัวนับเมื่อเริ่มอัดใหม่
                    ...
                    if TINYSA_ENABLED and tinysa_ser:
                        ...
                        current_monitoring_freq_hz_for_tinysa = int(float(FREQUENCY) * 1_000_000) if FREQUENCY.upper() != "SCAN" else TARGET_FREQUENCY_HZ
                        ...
                        setup_tinysa_for_measurement(tinysa_ser, current_monitoring_freq_hz_for_tinysa,TINYSA_SWEEP_POINTS, TINYSA_REPEAT_INTERVAL)
                ...
                if TINYSA_ENABLED and tinysa_ser and not tinysa_is_paused and (time.time() - last_tinysa_check_time >= 0.7):  # อ่านทุกๆ 1.5 วินาที (ปรับได้)
                    ...
                    # last_tinysa_check_time = time.time()
                    if strength is not None:
                        tinysa_reading_count += 1  # เพิ่มตัวนับ
                        # if not signal_strengths_db and len(frames) < (RATE / CHUNK * 0.5):  # ตัวอย่าง: ถ้าเป็นค่าแรกๆ และยังอัดไม่ถึง 0.5 วิ อาจจะข้าม
                        if tinysa_reading_count == 1 and len(frames) < (RATE / CHUNK * 0.5):
                            # log(f"📡 [TinySA] ข้ามการเก็บค่าสัญญาณรอบแรก: {strength:.2f} dBm (เพื่อรอความเสถียร)")
                            log(f"📡 [TinySA] ข้ามการเก็บค่าสัญญาณรอบแรก (count: {tinysa_reading_count}): {strength:.2f} dBm")
                        else:
                            signal_strengths_db.append(strength)
                            # log(f"📡 [TinySA] เก็บค่าสัญญาณ: {strength:.2f} dBm")
                            log(f"📡 [TinySA] เก็บค่าสัญญาณ (count: {tinysa_reading_count}): {strength:.2f} dBm")

                        # --- ทำการ Capture หน้าจอในรอบที่กำหนด ---
                        if TINYSA_CAPTURE_ENABLED and tinysa_reading_count == TINYSA_CAPTURE_ROUND:
                            log(f"📸 [TinySA] ถึงรอบที่ {TINYSA_CAPTURE_ROUND}, กำลัง Capture หน้าจอ...")
                            # อาจจะใช้ชื่อไฟล์ที่สื่อถึงไฟล์เสียงปัจจุบัน
                            capture_filename_base = f"tinysa_capture_{os.path.splitext(filename)[0]}" if 'filename' in locals() else "tinysa_capture"
                            capture_thread = threading.Thread(target=capture_tinysa_screen, args=(tinysa_ser, capture_filename_base), daemon=True)
                            capture_thread.start()
                            # ไม่ต้อง .join() เพื่อไม่ให้ block การบันทึกเสียง
                ...
                if stopped_by_silence or stopped_by_length:
                    ...
                    if TINYSA_ENABLED and tinysa_ser:
                        # --- ทำการ Capture หน้าจอครั้งสุดท้ายก่อน Pause (ถ้ายังไม่ได้ทำในรอบที่ต้องการ) ---
                        if TINYSA_CAPTURE_ENABLED and tinysa_reading_count < TINYSA_CAPTURE_ROUND and tinysa_reading_count > 0:  # Capture ถ้ายังไม่ถึงรอบ หรือยังไม่ได้ capture เลยแต่มีการอ่านค่าแล้ว
                            log(f"📸 [TinySA] กำลัง Capture หน้าจอครั้งสุดท้ายก่อนหยุดอัด...")
                            capture_filename_base = f"tinysa_capture_{os.path.splitext(filename)[0]}" if 'filename' in locals() else "tinysa_capture"
                            # เรียก capture โดยตรงถ้าไม่กังวลเรื่อง block หรือสร้าง thread
                            capture_tinysa_screen(tinysa_ser, capture_filename_base)
    ...
    if num_strength_samples > 0:
        ...
        meaningful_strengths_for_avg = []
        if num_strength_samples == 1:  # ถ้ามีค่าเดียว
            meaningful_strengths_for_avg = signal_strengths_db
            log(f"📊 [SignalCalc] มีตัวอย่าง 1 ค่า, ใช้ค่านั้นเลย")
        elif num_strength_samples == 2:  # ถ้ามี 2 ค่า (หลังจากข้ามค่าแรกไปแล้ว)
            meaningful_strengths_for_avg = signal_strengths_db  # อาจจะใช้ทั้งสองค่า หรือค่าที่สอง
            log(f"📊 [SignalCalc] มีตัวอย่าง 2 ค่า, คำนวณจากทั้งหมด")
        elif num_strength_samples >= 3:
            # ตัดค่าแรก (ที่อาจจะข้ามไปแล้วถ้า logic เดิมยังอยู่) และค่าสุดท้าย
            # ถ้า logic ข้ามค่าแรกทำงานไปแล้ว list นี้จะเริ่มจากค่าที่ "สอง" ของการอ่านจริง
            # การตัด [1:-1] จะเป็นการตัดค่า "ที่สอง" (ของจริง) และค่า "รองสุดท้าย" (ของจริง)
            # เพื่อความแม่นยำ อาจจะต้องปรับ logic การข้ามค่าแรกให้ชัดเจน
            # สมมติว่า signal_strengths_db คือ list ของค่าที่ "เก็บ" แล้ว (หลังข้ามค่าแรกไปแล้ว)
            if num_strength_samples >= 3:  # ถ้า list ที่เก็บแล้วมีอย่างน้อย 3 ค่า
                meaningful_strengths_for_avg = signal_strengths_db[0:-1]  # เอาตั้งแต่ตัวแรก ถึง "ก่อน" ตัวสุดท้าย
                log(f"📊 [SignalCalc] คำนวณค่าเฉลี่ยจาก {len(meaningful_strengths_for_avg)} ค่า (ตัดค่าสุดท้ายออก)")
            else:  # ถ้า list ที่เก็บแล้วมีน้อยกว่า 3 (เช่น 1 หรือ 2) ก็ใช้ทั้งหมด
                meaningful_strengths_for_avg = signal_strengths_db
                log(f"📊 [SignalCalc] มีตัวอย่างที่เก็บน้อยกว่า 3 ({len(meaningful_strengths_for_avg)} ค่า), คำนวณจากทั้งหมดที่เก็บ")

        if meaningful_strengths_for_avg:
            average_signal_db = sum(meaningful_strengths_for_avg) / len(meaningful_strengths_for_avg)

    if average_signal_db is not None:
        log(f"📊 ค่าเฉลี่ยความแรงสัญญาณสุดท้ายที่จะใช้: {average_signal_db:.2f} dBm")
    else:
        log(f"📊 ไม่สามารถคำนวณค่าเฉลี่ยความแรงสัญญาณได้ (ไม่มีตัวอย่างที่ใช้ได้)")
...
def capture_tinysa_screen(ser, output_filename_base="tinysa_capture"):
    """ส่งคำสั่ง capture และบันทึกข้อมูลหน้าจอเป็นไฟล์รูปภาพ"""
    if not TINYSA_ENABLED or not ser or not ser.is_open:
        return None

    global tinysa_is_paused  # เข้าถึงสถานะ pause/resume
    was_paused = tinysa_is_paused  # เก็บสถานะเดิม

    try:
        if tinysa_is_paused:
            resume_tinysa(ser)
            # อาจจะต้องรอให้มีการ sweep หรือ update หน้าจอเล็กน้อย
            time.sleep((TINYSA_REPEAT_INTERVAL / 1000.0 * TINYSA_SWEEP_POINTS) + 0.1)

        log(f"📸 [TinySA] Sending capture command...")
        ser.reset_input_buffer()
        ser.write(b"capture\r\n")  # ส่งคำสั่ง capture

        # รอข้อมูล binary กลับมา (ขนาดที่คาดหวัง: 320 * 240 * 2 bytes)
        expected_bytes = 320 * 240 * 2
        image_data = b''
        start_time = time.time()
        timeout_seconds = 5  # กำหนด timeout สำหรับการอ่านข้อมูลภาพ

        while len(image_data) < expected_bytes and (time.time() - start_time) < timeout_seconds:
            if ser.in_waiting > 0:
                bytes_to_read = min(ser.in_waiting, expected_bytes - len(image_data))
                image_data += ser.read(bytes_to_read)
            else:
                time.sleep(0.01)  # หน่วงเล็กน้อย

        if len(image_data) != expected_bytes:
            log(f"⚠️ [TinySA] Capture data incomplete. Expected {expected_bytes} bytes, got {len(image_data)} bytes.")
            # พยายามอ่าน prompt ทิ้งไป (ถ้ามี)
            time.sleep(0.2)
            if ser.in_waiting > 0: ser.read(ser.in_waiting)
            if was_paused and not tinysa_is_paused:  # ถ้าก่อนหน้า pause อยู่ ก็ pause กลับ
                pause_tinysa(ser)
            return None

        log(f"✅ [TinySA] Capture data received: {len(image_data)} bytes.")

        # สร้างรูปภาพจากข้อมูล binary ด้วย Pillow
        # ต้องทราบ format สีที่ TinySA ส่งมา (เช่น RGB565, BGR565)
        # สมมติว่าเป็น RGB565 (16-bit)
        # Pillow สามารถอ่าน raw data แล้วแปลงได้
        try:
            # สร้าง Image object จาก raw data
            # TinySA อาจจะส่งข้อมูลมาในรูปแบบ 16-bit (RGB565 หรือรูปแบบอื่น)
            # Pillow ใช้ 'RGB;16' สำหรับ RGB565 หรือ 'BGR;16' สำหรับ BGR565
            # คุณอาจจะต้องทดลองหรือดูจากเอกสาร TinySA ว่าเป็น format ใดแน่
            # หรือถ้าเป็น raw bytes ที่สามารถ map ตรงๆ ไปยัง RGB ได้ง่ายๆ ก็อาจจะไม่ต้องระบุ mode ซับซ้อน

            # วิธีที่ 1: ถ้าเป็น RGB565 (ต้องแน่ใจว่า Pillow รองรับการ decode แบบนี้โดยตรง)
            # img = Image.frombytes("RGB", (320, 240), image_data, "raw", "RGB;16")

            # วิธีที่ 2: สร้างภาพ grayscale ก่อน แล้วค่อยจัดการสี (ถ้า format สียังไม่ชัดเจน)
            # หรือถ้าข้อมูลเป็น grayscale 16-bit (ไม่น่าใช่สำหรับ screen dump ที่มีสี)

            # วิธีที่ 3: สร้างภาพ RGB จากข้อมูล 16-bit โดยการ unpack เอง (ซับซ้อนกว่า)
            # ตัวอย่างคร่าวๆ ถ้าเป็น RGB565 (R=5bit, G=6bit, B=5bit)
            ...
            for y in range(240):
                for x in range(320):
                    ...
                    if idx + 1 < len(image_data):
                        pixel_value = int.from_bytes(image_data[idx:idx + 2],
                                                     'little')  # หรือ 'big' ขึ้นอยู่กับ endianness
                        # แตก 16-bit RGB565 ออกเป็น R, G, B (8-bit)
                        r = (pixel_value >> 11) & 0x1F
                        g = (pixel_value >> 5) & 0x3F
                        b = pixel_value & 0x1F
                        # ขยายค่าสีจาก 5-bit หรือ 6-bit ไปเป็น 8-bit
                        pixels[x, y] = (r << 3, g << 2, b << 3)

            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
            filename = f"{output_filename_base}_{timestamp}.png"
            filepath = os.path.join(TINYSA_CAPTURE_SAVE_FOLDER, filename)
            img.save(filepath)
            log(f"🖼️ [TinySA] Screen capture saved to: {filepath}")

            # พยายามอ่าน prompt ทิ้งไป (ถ้ามี) หลัง capture
            time.sleep(0.2)  # ให้เวลากับอุปกรณ์ส่ง prompt
            if ser.in_waiting > 0:
                prompt_data = ser.read(ser.in_waiting)
                log(f"ℹ️ [TinySA] Trailing data after capture: {prompt_data.decode('ascii', errors='ignore').strip()}")

            if was_paused and not tinysa_is_paused:  # ถ้าก่อนหน้า pause อยู่ ก็ pause กลับ
                pause_tinysa(ser)
            elif not was_paused and tinysa_is_paused:  # ถ้าก่อนหน้า resume อยู่ ก็ resume กลับ (ถ้ามีการ pause ใน process)
                resume_tinysa(ser)

            return filepath
        except Exception as e_img:
            log(f"❌ [TinySA] Error processing image data: {e_img}")
            if was_paused and not tinysa_is_paused:
                pause_tinysa(ser)
            return None

    except serial.SerialException as e_serial:
        log(f"❌ [TinySA] Serial error during capture: {e_serial}")
        return None
    except Exception as e_capture:
        log(f"❌ [TinySA] Error during capture: {e_capture}")
        if was_paused and not tinysa_is_paused:  # พยายามคืนสถานะ pause/resume เดิม
            pause_tinysa(ser)
        return None
...
def upload_audio_and_text(original_wav_path, mp3_filepath, transcript, duration, engine_used, signal_dbm, worker_id_for_log=None):
    ...
    data = {
        ...
        'signal_dbm': f"{signal_dbm:.2f}" if signal_dbm is not None else None  # เพิ่ม field ใหม่
    }
